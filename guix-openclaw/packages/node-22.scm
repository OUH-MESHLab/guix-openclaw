...
;;; Vendored from GNU Guix gnu/packages/node.scm at commit d85dd388b8d
;;; (the last revision where node-lts was 22.14.0, before it was bumped to
;;; node 24).  guix upstream now ships only node 24.x, whose bundled npm
;;; 11.16 aborts every node-build-system build with "Exit handler never
;;; called!".  We keep a self-contained node 22.14.0 here — decoupled from
;;; guix's node-lts — so guix-openclaw's node packages build with npm 10.
;;; The only external patch (llhttp-ponyfill) was dropped upstream with the
;;; node-24 bump, so it is vendored alongside this file.
(define-module (guix-openclaw packages node-22)
  ...
  (package
    ...
    (arguments
     `(#:configure-flags '("--shared-cares"
                           ...
                           "--without-snapshot"
                           "--without-intl")
       ;; Skip the test suite: node 10's environment-sensitive tests fail on
       ;; current kernels/glibc, and we only need a working bootstrap node to
       ;; build llhttp and node 22 (not to certify node 10 itself).
       #:tests? #f
  ...
  (package
    ...
    (source (origin
              ...
              (patches (list (local-file
                              "patches/llhttp-ponyfill-object-fromentries.patch")))
              (modules '((guix build utils)))
              (snippet
               '(begin
                  ;; Fix imports for esbuild.
...
;;; Public, unambiguous name for consumers in this channel.
(define-public node-22 node-lts)
